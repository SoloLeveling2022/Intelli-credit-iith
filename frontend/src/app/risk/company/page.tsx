...
function CompanyScorecardContent() {
  ...
  if (!cin) {
    return (
      <PageShell title="Company Scorecard" description="Detailed credit risk analysis">
        <div className="c-bg-card rounded-xl border c-border p-12 text-center">
          <ShieldAlert className="w-12 h-12 c-text-3 mx-auto mb-4" />
          <p className="c-text-2 text-sm">No CIN provided. Please select a company from the Risk Assessment page.</p>
        </div>
      </PageShell>
    );
  }
...
export default function CompanyScorecardPage() {
  return (
    <Suspense
      fallback={
        <PageShell title="Company Scorecard" description="Loading...">
          <div className="flex items-center justify-center py-20">
            <div className="animate-spin rounded-full h-8 w-8 border-2 border-t-transparent" style={{ borderColor: "var(--text-secondary)", borderTopColor: "transparent" }} />
          </div>
        </PageShell>
