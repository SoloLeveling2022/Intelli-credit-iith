...
import { getCompanyRisks, getCompanyRiskSummary } from "@/lib/api";
...
export default function RiskPage() {
  const [companies, setCompanies] = useState<CompanyRisk[]>([]);
  const [filtered, setFiltered] = useState<CompanyRisk[]>([]);
  ...
  useEffect(() => {
    getCompanyRisks()
      .then((data) => {
        const payload = data as CompanyRiskApiResponse;
        ...
        const c: CompanyRisk[] = rawCompanies.map((v) => ({
          cin: v.cin || "",
          legal_name: v.legal_name || v.company_name || "",
          ...
          risk_score:
            typeof v.risk_score === "number"
              ...
              : 0,
          filing_rate:
            typeof v.filing_rate === "number"
              ? v.filing_rate
              : 0,
          mismatch_count:
            typeof v.mismatch_count === "number"
              ? v.mismatch_count
              : 0,
          total_invoices:
            typeof v.total_invoices === "number"
              ? v.total_invoices
              : 0,
          circular_trade_flag:
            typeof v.circular_trade_flag === "boolean"
              ? v.circular_trade_flag
              : Boolean(v.shell_company_flag),
          risk_factors: Array.isArray(v.risk_factors) ? v.risk_factors : [],
          trade_partners:
            typeof v.trade_partners === "number"
              ? v.trade_partners
              : undefined,
        }));
  ...
  useEffect(() => {
    ...
    if (selectedCompany) {
      f = f.filter((v) => v.cin === selectedCompany);
    }
    setFiltered(f);
  }, [companies, filterLevel, selectedCompany]);

  const companyOptions: DropdownOption[] = useMemo(() => {
    return companies.map((v) => ({
      value: v.cin,
      label: v.legal_name || v.trade_name || v.cin,
      sublabel: v.cin,
    }));
  }, [companies]);

  const loadAISummary = async (cin: string) => {
    setAiLoading(true);
    setAiPanel(null);
    try {
      const data = (await getCompanyRiskSummary(cin)) as AISummary;
      setAiPanel(data);
    } catch {
      // pass
    }
  ...
  const riskCounts = {
    CRITICAL: companies.filter((v) => v.risk_level === "CRITICAL").length,
    HIGH: companies.filter((v) => v.risk_level === "HIGH").length,
    MEDIUM: companies.filter((v) => v.risk_level === "MEDIUM").length,
    LOW: companies.filter((v) => v.risk_level === "LOW").length,
  };
    ...
    >
      {loading ? (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold c-text truncate">
                      {company.legal_name || company.trade_name || "Unknown"}
                    </p>
                  ...
                  <span
                    className={`text-xs font-bold px-2 py-1 rounded shrink-0 ml-2 ${riskColor(company.risk_level)}`}
                  >
                    {company.risk_level}
                  </span>
                </div>
                ...
                <div className="mb-3">
                  <div className="flex justify-between text-xs c-text-2 mb-1">
                    ...
                    <span>{company.risk_score.toFixed(0)}/100</span>
                  </div>
                  <div className="w-full c-bg-dark rounded-full h-2">
                    <div
                      ...
                      style={{ width: `${Math.min(100, company.risk_score)}%` }}
                    />
                  </div>
                </div>
                ...
                {/* Risk factors */}
                {company.risk_factors.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {company.risk_factors.map((f, i) => (
                      <span
                        key={i}
                ...
                <button
                  onClick={() => loadAISummary(company.cin)}
                ...
                <Link
                  href={`/risk/company?cin=${company.cin}`}
        ...
        <div className="c-bg-card rounded-xl border c-border p-12 text-center">
          ...
          <p className="c-text-2 font-medium mb-1">No company data</p>
          <p className="c-text-3 text-sm">
            Upload documents and run credit appraisal first.
          </p>
        </div>
      )}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>
                      {aiPanel.company.legal_name || aiPanel.company.trade_name}
                    </p>
                  ...
                  <span
                    className={`text-xs font-bold px-2 py-1 rounded ${riskColor(aiPanel.company.risk_level)}`}
                  >
                    {aiPanel.company.risk_level}
                  </span>
                </div>
