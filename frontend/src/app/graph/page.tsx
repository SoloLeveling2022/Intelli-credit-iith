...
export default function GraphPage() {
  ...
  /* Circular trades */
  const loadShellCompanies = async () => {
    try {
      const data = (await getShellCompanies()) as ShellCompany[];
      setShellCompanies(Array.isArray(data) ? data : []);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="hidden sm:inline">
                {showCircular ? "Hide Shell Companies" : "Shell Companies"}
              </span>
        ...
        {showCircular && shellCompanies.length > 0 && (
          <div className="flex flex-col gap-2 p-3 bg-red-500/5 border border-red-500/15 rounded-xl">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-red-400 flex items-center gap-1.5">
                <AlertTriangle className="w-3.5 h-3.5" />
                {shellCompanies.length} shell company pattern{shellCompanies.length > 1 ? "s" : ""} detected
              </span>
            ...
            <div className="flex flex-wrap gap-2">
              {shellCompanies.slice(0, 3).map((company, i) => (
