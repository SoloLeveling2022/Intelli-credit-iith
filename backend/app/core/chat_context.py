...
def build_user_context(gstin: str) -> str:
    ...
    try:
        ...
        all_vendor_risks = calculate_all_vendor_risks()
        user_risk = next((v for v in all_vendor_risks if v["gstin"] == gstin), None)
        if user_risk:
            sections.append(
                f"## Your Risk Profile\n"
                f"  - Risk Score: {user_risk['risk_score']}/100 ({user_risk['risk_level']})\n"
                f"  - Outward Invoices: {user_risk['outward_invoices']}\n"
                f"  - Inward Invoices: {user_risk['inward_invoices']}"
            )

        partner_risks = [v for v in all_vendor_risks if v["gstin"] != gstin and v["risk_score"] > 20][:5]
        if partner_risks:
            risk_lines = [
                f"  - {v['name']} ({v['gstin']}) — Score: {v['risk_score']}, Level: {v['risk_level']}"
...
def get_smart_suggestions(gstin: str) -> list[str]:
    ...
    if len(suggestions) < 4:
        suggestions.append("What are the key credit and GST compliance risks I should be aware of?")
    if len(suggestions) < 5:
        suggestions.append("How can I improve my credit profile and GST filing compliance?")
